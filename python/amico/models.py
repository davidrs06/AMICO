...
class CylinderTimedepZeppelinBall( BaseModel ) :
    ...
    The intra-cellular contributions from within the axons are modeled as "cylinders"
    with specific radii (Rs) and a given axial diffusivity (d_par).
    Extra-cellular contributions are modeled as time dependent tensors with the same axial diffusivity
    as the cylinders (d_par) and whose perpendicular diffusivities are calculated from of a set of
    characteristic coefficients A and bulk diffusions Dinf as well as the shell's diffusion times.
    Isotropic contributions are modeled as tensors with isotropic diffusivities (d_ISOs).
    ...
    References
    ----------
    .. [1] De Santis et al. (2012) Including diffusion time dependende in the extra-axonal space improves
           in vivo estimates of axonal diameter and density in human white matter. NeuroImage, 59: 2241-54
    """
    ...
    def __init__( self ) :
        ...
        self.d_par  = 0.6E-3                                                         # Parallel diffusivity [mm^2/s]
        self.Rs     = np.concatenate( ([0.01],np.linspace(0.5,8.0,20.0)) ) * 1E-6    # Radii of the axons [meters]
        self.timedep_A = np.linspace(0.1,1,5)*1E-6                                   # Characteristic coefficient for timedependency [mm^2]
        self.timedep_Dinf = np.linspace(0.01,0.2,5)*1E-3                             # Bulk diffusion for timedependency [mm^2/s]
        self.d_ISOs = np.array( [ 2.0E-3 ] )                                         # Isotropic diffusivitie(s) [mm^2/s]
        self.isExvivo  = False                                                       # Add dot compartment to dictionary (exvivo data)
        self.singleb0  = True                                                        # Merge b0 images into a single volume for fitting


    def set( self, d_par, Rs, ICVFs, d_ISOs, A, Dinf ) :
        self.d_par  = d_par
        self.Rs     = np.array(Rs)
        self.ICVFs  = np.array(ICVFs)
        self.d_ISOs = np.array(d_ISOs)
    ...
    def generate( self, out_path, aux, idx_in, idx_out ) :
        ...
        nATOMS = len(self.Rs) + len(self.timedep_A)*len(self.timedep_Dinf) + len(self.d_ISOs)
        progress = ProgressBar( n=nATOMS, prefix="   ", erase=True )
    ...
    def resample( self, in_path, idx_out, Ylm_out ) :
        ...
        KERNELS['model'] = self.id
        KERNELS['wmr'] = np.zeros( (len(self.Rs),181,181,self.scheme.nS,), dtype=np.float32 )
        KERNELS['wmh'] = np.zeros( (len(self.timedep_A)*len(self.timedep_Dinf),181,181,self.scheme.nS,), dtype=np.float32 )
        KERNELS['iso'] = np.zeros( (len(self.d_ISOs),self.scheme.nS,), dtype=np.float32 )
        ...
        # Zeppelin(s)
        for i in xrange(len(self.timedep_A)*len(self.timedep_Dinf)):
            lm = np.load( pjoin( in_path, 'A_%03d.npy'%progress.i ) )
            KERNELS['wmh'][i,:,:,:] = amico.lut.resample_kernel( lm, self.scheme.nS, idx_out, Ylm_out, False )
            progress.update()
    ...
    def fit( self, y, dirs, KERNELS, params ) :
        nD = dirs.shape[0]
        n1 = len(self.Rs)
        n2 = len(self.timedep_A)*len(self.timedep_Dinf)
        n3 = len(self.d_ISOs)
...
class NODDI( BaseModel ) :
    ...
    def generate( self, out_path, aux, idx_in, idx_out ):
        ...
        for kappa in IC_KAPPAs:
            signal_ic = synth_meas_watson_SH_cyl_neuman_PGSE( np.array([self.dPar*1E-6, 0, kappa]), protocolHR['grad_dirs'], np.squeeze(protocolHR['gradient_strength']), np.squeeze(protocolHR['delta']), np.squeeze(protocolHR['smalldel']), np.array([0,0,1]), 0 )

            for v_ic in self.IC_VFs:
                dPerp = self.dPar*1E-6 * (1 - v_ic)
                signal_ec = synth_meas_watson_hindered_diffusion_PGSE( np.array([self.dPar*1E-6, dPerp, kappa]), protocolHR['grad_dirs'], np.squeeze(protocolHR['gradient_strength']), np.squeeze(protocolHR['delta']), np.squeeze(protocolHR['smalldel']), np.array([0,0,1]) )
        ...
        # Isotropic
        signal = synth_meas_iso_GPD( self.dIso*1E-6, protocolHR)
        lm = amico.lut.rotate_kernel( signal, aux, idx_in, idx_out, True )
        np.save( pjoin( out_path, 'A_%03d.npy'%progress.i) , lm )
        progress.update()
...
class WatsonZeppelinBall( BaseModel ) :
    """Implements the WatsonZeppelinBall model [1].
    ...
    References
    ----------
    .. [1] Zhang et al. (2011) Axon diameter mapping in the presence of 
           orientation dispersion with diffusion MRI. NeuroImage, 56: 1301-15
    """
    def __init__( self ):
        self.id         = "WatsonZeppelinBall"
        self.name       = "WatsonZeppelinBall"
        self.maps_name  = [ 'a', 'ICVF', 'Kappa', 'ISOVF' ]
        self.maps_descr = [ 'Mean diameter index', 'Intra-cellular volume fraction', 'Orientation dispersion', 'Isotropic volume fraction' ]

        self.dPar      = 0.6E-3
        self.dIso      = 2.0E-3
        self.Rs        = np.concatenate( ([0.01],np.linspace(0.5,8.0,20.0)) ) * 1E-6
        self.IC_VFs    = np.linspace(0.3,0.9,7)
        self.IC_KAPPAs = np.array([128, 64, 32, 16, 8, 4])
        self.isExvivo  = True


    def set( self, dPar, dIso, IC_VFs, IC_KAPPAs, isExvivo ):
        self.dPar      = dPar
        self.dIso      = dIso
        self.IC_VFs    = np.array( IC_VFs )
        self.IC_KAPPAs    = np.array( IC_KAPPAs )
        self.isExvivo  = isExvivo
    ...
    def generate( self, out_path, aux, idx_in, idx_out ):
        ...
        nATOMS = len(self.Rs)*len(self.IC_KAPPAs)*len(self.IC_VFs) + 1
        progress = ProgressBar( n=nATOMS, prefix="   ", erase=True )

        # Coupled contributions
        for kappa in self.IC_KAPPAs:
            for R in self.Rs:
                signal_ic = synth_meas_watson_SH_cyl_neuman_PGSE( np.array([self.dPar*1E-6, R, kappa]), protocolHR['grad_dirs'], np.squeeze(protocolHR['gradient_strength']), np.squeeze(protocolHR['delta']), np.squeeze(protocolHR['smalldel']), np.array([0,0,1]), scipy.special.jnp_zeros(1, 6) ) # number of roots set to default found in NODDI-matlab function root = BesselJ_RootsCyl(starts) (20 starting points which returns 6 roots)
    ...
    def resample( self, in_path, idx_out, Ylm_out ):
        ...
        KERNELS['wm']    = np.zeros( (nATOMS-1,181,181,self.scheme.nS), dtype=np.float32 )
        KERNELS['iso']   = np.zeros( self.scheme.nS, dtype=np.float32 )
        KERNELS['R']     = np.zeros( nATOMS-1, dtype=np.float32 )
        KERNELS['kappa'] = np.zeros( nATOMS-1, dtype=np.float32 )
        KERNELS['icvf']  = np.zeros( nATOMS-1, dtype=np.float32 )
        KERNELS['norms'] = np.zeros( (self.scheme.dwi_count, nATOMS-1) )
        ...
        # Coupled contributions
        for i in xrange( len(self.IC_KAPPAs) ):
            for R in xrange(len(self.Rs)):
                for j in xrange( len(self.IC_VFs) ):
                    lm = np.load( pjoin( in_path, 'A_%03d.npy'%progress.i ) )
                    idx = progress.i - 1
                    KERNELS['wm'][idx,:,:,:] = amico.lut.resample_kernel( lm, self.scheme.nS, idx_out, Ylm_out, False )
                    KERNELS['R'][idx] = self.Rs[R]
                    KERNELS['kappa'][idx] = self.IC_KAPPAs[i]
                    KERNELS['icvf'][idx]  = self.IC_VFs[j]
                    KERNELS['norms'][:,idx] = 1 / np.linalg.norm( KERNELS['wm'][idx,0,0,self.scheme.dwi_idx] ) # norm of coupled atoms (for l1 minimization)
                    progress.update()
    ...
    def fit( self, y, dirs, KERNELS, params ) :
        ...
        # prepare DICTIONARY from dir and lookup tables
        nATOMS_aniso = len(self.Rs)*len(self.IC_KAPPAs)*len(self.IC_VFs)
        nATOMS_iso = 1
        nATOMS = nATOMS_aniso + nATOMS_iso
        if self.isExvivo == True :
            nATOMS += 1
        ...
        return [a, v, k, fISO], dirs, x, A


    def scheme2noddi( self, scheme ):
        if scheme.version == 0:
            raise RuntimeError( '%s model not suited for BVECTOR scheme (requires G, Delta and delta values)' % self.name )
        protocol = {}
        protocol['pulseseq'] = 'PGSE'
        protocol['schemetype'] = 'multishellfixedG'
        protocol['teststrategy'] = 'fixed'
...
def cyl_neuman_le_perp_PGSE( d, R, G, delta, smalldel, roots ):
    ...
    l_q=G.shape[0]
    l_a=1 # function only used for a single radius in the framework of WatsonZeppelinBall model
    k_max=len(roots)

    R_mat=np.ones((l_q, 1, k_max))*R
    R_matSq=R_mat**2

    root_m=roots[None,None,:]
    alpha_mat=np.tile(root_m,[l_q*l_a, 1, 1])/R_mat
    amSq=alpha_mat**2
    amP6=amSq**3

    deltamx=delta[:,None]
    deltamx_rep = np.tile(deltamx[:,:,None],[1, k_max])

    smalldelmx=smalldel[:,None]
    smalldelmx_rep = np.tile(smalldelmx[:,:,None],[1, k_max])
    ...
    testinds = np.argwhere(sumterms[:,0,-1]>0)
    test = sumterms[testinds,0]/sumterms[testinds,-1]
    if test.min() <1E4:
        warnings.warn('Ratio of largest to smallest terms in Neuman model sum is <1E4.  May need more terms.')

    s = sumterms.sum(axis=2)
    if s.min() < 0:
        warnings.warn('Negative sums found in Neuman sum.  Setting to zero.')
        s[s<0]=0
